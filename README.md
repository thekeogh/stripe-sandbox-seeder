...
Open <http://127.0.0.1:43187>, paste a **sandbox secret API key** into **Stripe connection**, and click **Connect**. Select a product and recurring price, configure your customers, and click **Seed customers**.
...
- Separate customer and subscription metadata editors. Enter inserts and focuses the next row; plus and minus insert/remove rows. Stripe's metadata limits and duplicate keys are validated.
- Products, active recurring prices and valid compatible coupons loaded from Stripe, including pagination. This uses the Products and Prices APIs, never the Plans API. The selected price uses quantity 1, except metered prices, which omit quantity.
