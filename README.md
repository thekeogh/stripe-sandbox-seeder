...
In the app, paste a **sandbox secret API key** into **Stripe connection**, and click **Connect**. Select a product and recurring price (or **Custom price**), configure your customers, and click **Seed customers**.

You can alternatively set `STRIPE_SECRET_KEY` in `.env.local` using `.env.example`, then connect with an empty key field. The UI key takes precedence. Live keys can look up account details, but catalogue and seeding calls reject them. The app binds to localhost and is intended for personal local use; it does not include hosted multi-user authentication.

The connection card identifies **TEST MODE** or **LIVE MODE** from the key prefix before connection. A live key gets a prominent red warning and can only make a read-only account lookup; catalogue and seeding calls remain blocked. When Stripe permits it, the app shows the account ID, dashboard display name, business name, email, country, default currency, business type and account type. Some fields may be blank, and Stripe may not return the separate sandbox label. Account details are fetched again on reload when a saved key reconnects; they are not saved in browser storage. Restricted keys can still load the catalogue if the account lookup lacks permission.
...
- Company names by default, or first and last names, generated using Faker. Choose **Custom** to enter an exact customer name; all customers in a batch then share that name while their email addresses remain Faker-generated. The custom name is saved locally and cleared by Reset Everything.
...
- **Generated email** remains the default and creates a distinct address for each customer, with an optional domain override. Choose **Custom email** to enter one complete address reused across the batch. The mode, domain override and custom address are all saved locally and cleared by Reset Everything.
...
- **Custom price** is available in the recurring price selector and is selected automatically when a product has no recurring prices. Choose USD, GBP, EUR or CAD, a billing interval (year by default), and an amount per unit. The amount field shows the currency symbol and groups thousands as you type; enter pounds/dollars/euros, including up to two decimal places. The seeder converts this to the integer minor-unit `unit_amount` and sends inline `items[0][price_data]` with the selected product. These settings persist locally and reset with the rest of the form. Stripe may reject amounts outside its account and currency limits.
...
Optional **Free trial (days)** applies the entered positive whole number to every subscription via `trial_period_days`. Leave it blank for no trial. It is saved with your preferences and cleared by Reset Everything. Card payment methods are still attached before creating trial subscriptions.
